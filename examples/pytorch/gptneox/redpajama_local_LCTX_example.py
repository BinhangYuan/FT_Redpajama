...
def main():
    ...
    parser.add_argument('--ckpt_path', type=str, default='/workspace/FasterTransformer/build/model/ft-RedPajama-32KCTX-7B/1-gpu',
                        help='path to the checkpoint file.')
    parser.add_argument('--tokenizer_path', type=str, default='togethercomputer/RedPajama-INCITE-7B-Base',
    ...
    parser.add_argument('--sample_input_file', type=str, default='/workspace/FasterTransformer/build/model/raw_wiki_text.txt',
    ...
    parser.add_argument('--prompt_seq_length', type=int, default=1920, help='How many tokens in prefill?')
    parser.add_argument('--generate_seq_length', type=int, default=128, help='How many tokens in token generation?')
    ...
    output_len = args.generate_seq_length
    ...
    if args.sample_input_file:  # conditional case
        with open(args.sample_input_file, "r") as f:
            ...
            batch_size = 1
        start_ids = [torch.tensor(tokenizer.encode(contexts[0]), dtype=torch.int32, device=device)]
        print(f"Raw start_ids[0] shape: {start_ids[0].shape}")
        
        input_length = start_ids[0].shape[0]
        print(f"Raw input context length: {input_length}")
        
        if input_length > args.prompt_seq_length:
            start_ids[0] = start_ids[0][0:args.prompt_seq_length]
